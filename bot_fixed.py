...
TICKET_CATEGORY_ID = 1537485981446447145
...
@bot.tree.command(
    name="themsp",
    description="Thêm sản phẩm vào shop",
)
...
@bot.tree.command(
    name="sua_sp",
    description="Sửa giá hoặc kho sản phẩm",
)
@app_commands.describe(
    ma="Mã sản phẩm",
...
async def sua_sp(
    ...
    if gia is None and kho is None:
        await interaction.response.send_message(
            "❌ Bạn phải nhập giá mới hoặc kho mới.",
            ephemeral=True,
        )
        return
...
async def on_ready():
    ...
    # Chỉ add persistent views một lần.
    ...
    try:
        ...
        print(f"❌ Không thể sync command: {e}")
...
if not TOKEN:
    raise RuntimeError(
        "❌ Chưa đặt biến môi trường TOKEN."
    )
