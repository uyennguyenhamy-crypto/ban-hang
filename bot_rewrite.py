...
async def refresh_shop(guild: discord.Guild):
    ...
    try:
        ...
        print(
            "[SHOP] Bot thiếu quyền View Channel / Send Messages / "
            "Manage Messages."
        )
    except discord.HTTPException as e:
...
async def themsp(interaction: discord.Interaction, ma: str, ten: str, gia: str, kho: int):
    # Trả ACK ngay lập tức để Discord không hiện "Ứng dụng không phản hồi".
    ...
    if not re.fullmatch(r"[a-z0-9_-]{1,30}", code):
        await interaction.followup.send("❌ Mã chỉ được dùng chữ thường, số, `_` hoặc `-`.", ephemeral=True)
        return

    price = parse_price(gia)
    if price <= 0 or kho < 0:
        await interaction.followup.send("❌ Giá hoặc số lượng không hợp lệ.", ephemeral=True)
        return

    if not ten.strip():
        await interaction.followup.send("❌ Tên sản phẩm không được để trống.", ephemeral=True)
        return
    ...
    products[code] = {"name": ten.strip(), "price": price, "stock": kho}
    save_products()
...
async def sua_sp(
    ...
    if gia is None and kho is None:
        await interaction.followup.send("❌ Bạn phải nhập giá mới hoặc kho mới.", ephemeral=True)
        return
    ...
    if kho is not None:
        if kho < 0:
            await interaction.followup.send("❌ Kho không được nhỏ hơn 0.", ephemeral=True)
            return
        item["stock"] = kho
...
async def xemsp(interaction: discord.Interaction):
    ...
    if not products:
        await interaction.followup.send("📦 Shop hiện chưa có sản phẩm.", ephemeral=True)
        return
...
@bot.tree.command(name="shop", description="Tạo lại bảng shop")
async def shop(interaction: discord.Interaction):
    ...
    if interaction.guild is None:
        await interaction.followup.send("❌ Không xác định được server.", ephemeral=True)
        return

    await refresh_shop(interaction.guild)
    await interaction.followup.send("✅ Đã cập nhật bảng shop.", ephemeral=True)
...
@bot.tree.error
def _tree_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    # Decorator registration is intentionally avoided here; the actual async
    # handler below is installed through bot.tree.on_error.
    pass
...
async def _handle_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    ...
    text = "❌ Bot gặp lỗi khi xử lý lệnh. Hãy thử lại."
    try:
        ...
        print(f"[ERROR RESPONSE] {type(send_error).__name__}: {send_error}")
...
async def on_ready():
    ...
    if not getattr(bot, "_views_loaded", False):
        ...
        print("✅ Persistent views đã được load.")

    try:
        synced = await bot.tree.sync()
        print(f"✅ Đã sync {len(synced)} slash command.")
        print("📋 Commands:", ", ".join(c.name for c in bot.tree.get_commands()))
    except Exception as e:
        print(f"❌ Không thể sync command: {type(e).__name__}: {e}")
...
if not TOKEN:
    raise RuntimeError("❌ Chưa đặt biến môi trường TOKEN.")
